...
async def process_image_analysis(
    ...
    try:
        ...
        topic = (f'tic_claim_{str(headers.invitationCode)}' 
                ...
                else f'tic_claim_{str(new_image.keycloak_user_id)}')
        ...
        try:
            ...
            try:
                results_json = json.dumps(mapped_results) if mapped_results else json.dumps([])
            ...
            new_image.results = json.dumps([])  # Ensure results is always valid JSON
            db.commit()
